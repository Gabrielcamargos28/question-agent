...
Para quantificar o impacto técnico e econômico da arquitetura proposta na ingestão de dados legados do PLURI, definimos as seguintes Perguntas de Pesquisa (PP):
\begin{itemize}
\item \textbf{PP1: Qualidade de Extração Semântica.} Qual o grau de acerto do agente em classificar e correlacionar a disciplina e assunto corretos com o banco de dados legados da plataforma?
\item \textbf{PP2: Conformidade Sintática com a API.} Como o pipeline se comporta na prevenção de falhas de desserialização e violação de restrições na API de destino?
\item \textbf{PP3: Eficiência Econômica e Resiliência.} Como a introdução de técnicas de sliding-window chunking e controle de cota atenua erros de rede e otimiza custos?
\item \textbf{PP4: Redução de Redundância Multimídia.} Qual o impacto do algoritmo de hashing MD5 na eliminação de imagens redundantes?
\end{itemize}

Para responder a essas perguntas de pesquisa, empregamos as seguintes métricas quantitativas:
\begin{enumerate}
\item \textbf{F1-Score de Mapeamento Semântico ($F_1$):} Mede a acurácia da classificação de disciplinas e assuntos comparando a saída estruturada do agente com um conjunto de dados padrão (ground truth) anotado manualmente por coordenadores pedagógicos:
\begin{equation}
F_1 = 2 \times \frac{\text{Precisão} \times \text{Revogação}}{\text{Precisão} + \text{Revogação}}
\end{equation}
\item \textbf{Taxa de Sucesso de Cadastro (TSC):} Mede a porcentagem de requisições de cadastro que retornaram resposta HTTP 200/201 (Sucesso) em relação ao total de tentativas submetidas à API REST:
\begin{equation}
\text{TSC} = \frac{\text{Cadastros Bem-sucedidos}}{\text{Total de Ingestões Submetidas}} \times 100\%
\end{equation}
\item \textbf{Custo por Ingestão (CPI):} Representa o valor financeiro médio (USD) cobrado para processar um lote padrão de 1.000 questões, permitindo avaliar a sustentabilidade operacional a longo prazo com base nas chamadas de tokens da API Vertex AI.
\item \textbf{Taxa de Redução de Upload (TRU):} Percentual de imagens duplicadas descartadas antes da etapa de upload ao storage:
\begin{equation}
\text{TRU} = \left( 1 - \frac{\text{Imagens Enviadas ao Storage}}{\text{Imagens Totais no PDF}} \right) \times 100\%
\end{equation}
\end{enumerate}
